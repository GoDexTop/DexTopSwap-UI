...
interface OverviewResponse {
  dextopFactories: dextopFactory[]
  factories?: dextopFactory[]
}
/**
...
const getOverviewData = async (
  ...
  const factoryString = checkIsStableSwap() ? `factories` : `dextopFactories`
  try {
...
const formatdextopFactoryResponse = (rawdextopFactory?: dextopFactory[]) => {
  if (rawdextopFactory) {
    return rawdextopFactory.reduce(
      (acc, cur) => {
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const overviewData = formatdextopFactoryResponse(data?.dextopFactories)
      const overviewData24 = formatdextopFactoryResponse(data24?.dextopFactories)
      const overviewData48 = formatdextopFactoryResponse(data48?.dextopFactories)
...
export const fetchProtocolData = async (chainName: MultiChainName, block24: Block, block48: Block) => {
  ...
  if (data?.factories && data.factories.length > 0) data.dextopFactories = data.factories
  if (data24?.factories && data24.factories.length > 0) data24.dextopFactories = data24.factories
  if (data48?.factories && data48.factories.length > 0) data48.dextopFactories = data48.factories
