...
/**
 * The dextopswapMulticall contract has added functionality for limiting the amount of gas
 ...
 *
 * @export
 * @class dextopMulticallProvider
 */
export class dextopMulticallProvider extends IMulticallProvider<dextopMulticallConfig> {
  ...
  constructor(protected chainId: ChainId, protected provider: PublicClient, protected gasLimitPerCall = 1_000_000) {
    super()
    const multicallAddress = dextop_MULTICALL_ADDRESSES[this.chainId]

    if (!multicallAddress) {
      throw new Error(`No address for dextopswap Multicall Contract on chain id: ${chainId}`)
    }
  ...
  public async callSameFunctionOnMultipleContracts<TFunctionParams extends any[] | undefined, TReturn = any>(
    ...
    const {
      ...
      abi: IMulticallABI,
      address: dextop_MULTICALL_ADDRESSES[this.chainId],
  ...
  public async callSameFunctionOnContractWithMultipleParams<TFunctionParams extends any[] | undefined, TReturn>(
    params: CallSameFunctionOnContractWithMultipleParams<TFunctionParams, dextopMulticallConfig>,
  ): Promise<{
    blockNumber: bigint
    results: Result<TReturn>[]
  ...
  public async callMultipleFunctionsOnSameContract<TFunctionParams extends any[] | undefined, TReturn>(
    params: CallMultipleFunctionsOnSameContractParams<TFunctionParams, dextopMulticallConfig>,
  ): Promise<{
    blockNumber: bigint
    results: Result<TReturn>[]
    approxGasUsedPerSuccessCall: number
